...
global current_user, selected_course, selected_stream, session_start_time, session_end_time
...
class Functionalities(tk.Tk, Data_Verification):
    ...
    def login_data_validation(self, username, password):
        ...
        # Create another elif condition here to contact with DataBase and validate login credentials.
            ...
            IcdeDataBase.update_lastloggedin_time(obj, current_user)
    ...
    def signup_data_validation(self, firstname, lastname, username, phone_number, password, reentered_password, gender,
                               stream, date_of_birth, terms_accecpted):
        error_shown = False
        ...
        if not error_shown:
            for i in date_of_birth[8:10]:
                if ord(i) < 48 or ord(i) > 57:
                    ...
                    error_shown = True
        if not error_shown:
            if (int(date_of_birth[0:4]) > datetime.date.today().year) or (int(date_of_birth[5:7]) > 12) or (
                    int(date_of_birth[8:10]) > 31):
                tk.messagebox.showwarning("Error", "Invalid DOB")
                error_shown = True
        if not error_shown:
            if terms_accecpted == 0:
                tk.messagebox.showwarning("Error", "Please accept the privacy policy rules")
                error_shown = True
        if not error_shown:
    ...
    def generateotp(self, email):
        try:
            if collection.find_one({"Email": email}) is None:
                ...
                s.starttls()
                s.login("Icdeskillstack@gmail.com", "*123Icdeskillstack")
                self.otp = random.randint(1000, 9999)
                self.otp = str(self.otp)
                s.sendmail("Icdeskillstack@gmail.com", email, self.otp)
                messagebox.showinfo("Send OTP via Email", f"OTP sent to {email}")
                s.quit()
    ...
    def authorisefn(self,email,otp):
        ret = True if self.otp == str(otp) else False# add otp verification
        if ret:
            tk.messagebox.showinfo("Message", "User authorization successful")
            self.show_frame(self.passwordReset_frame)
        else:
            tk.messagebox.showwarning("Error", "User authorization unsuccessful")
        ...
        if not error_shown:
            for i in DOB[8:10]:
                if ord(i) < 48 or ord(i) > 57:
                    ...
                    error_shown = True
        if not error_shown:
            if (int(DOB[0:4]) > datetime.date.today().year) or (int(DOB[5:7]) > 12) or (
                    int(DOB[8:10]) > 31):
                tk.messagebox.showwarning("Error", "Invalid DOB")
                error_shown = True
        if not error_shown:
...
class Initialization(tk.Tk):
    ...
    def setup(self):
        ...
        self.authenticate_frame = tk.Frame(self)
...
class ICDEUser(Initialization, Functionalities):
    ...
    def __init__(self):
        ...
        def registrationpage():
            ...
            #
            terms_accecpted = tk.IntVar()
            button = tk.Checkbutton(self.signup_frame, text='I agree to all the ICDE - [SkillStack] privacy policy terms', font=("bold", 12), variable= terms_accecpted).place(x=130, y=550)
            tk.Button(self.signup_frame, text='Submit', width=20, bg="SteelBlue2", fg='white', font=("bold", 12),
                      command=lambda: [
                          self.signup_data_validation(firstname_entry.get(), lastname_entry.get(), email_entry.get(),
                                                      ...
                                                      gen_var.get(), stream.get(), date_of_birth.get(), terms_accecpted.get())]).place(x=180,
                                                                                                                y=600)
            tk.Button(self.signup_frame, text="Back to Home page", width=20, bg="SteelBlue3", fg='white',
                      font=("bold", 12),
                      command=lambda: self.show_frame(self.home_frame)).place(x=180, y=650)

            tk.Label(self.signup_frame, text="Already have an account?", width=50, font=("bold", 18)).place(x=770, y=150)
            tk.Button(self.signup_frame, text="Sign in here", width=20, bg="SteelBlue2", fg='white',
                      font=("bold", 12),
                      command=lambda: self.show_frame(self.login_frame)).place(x=1020, y=200)
            tk.Label(self.signup_frame, text="Happy Learning :)", font=("TrebuchetMS", 26, "bold"),
                     fg="DodgerBlue4").place(x=970,y=330)
        ...
        def loginpage():
            ...
            tk.Button(self.login_frame, text="Login", width=30, height=1, bg="SteelBlue1", font=("Cooper Black", 12),
                      command=lambda: [self.loguser(username_entry.get()), AppMainPage(),
                                       self.login_data_validation(username_entry.get(), password_entry.get())]).pack()
            tk.Label(self.login_frame, text="").pack()
            tk.Button(self.login_frame, text="Forgot Password", width=30, height=1, bg="SteelBlue2",
                      font=("Cooper Black", 12),
                      command=lambda: [self.refresh(self.authenticate_frame), authenticatepage(),
                                       self.show_frame(self.authenticate_frame)]).pack()
            tk.Label(self.login_frame, text="").pack()
        ...
        def authenticatepage():
            self.refresh(self.authenticate_frame)
            tk.Label(self.authenticate_frame, text="Email Authentication Page", font="helvetica, 20").place(x=300, y=40)
            tk.Label(self.authenticate_frame, text="").pack()
            tk.Label(self.authenticate_frame, text="E-mail", font="helvetica, 10").place(x=60, y=160)
            Email_entry = tk.Entry(self.authenticate_frame, width=40, font=("bold", 12))
            Email_entry.place(x=270, y=160)
            tk.Label(self.authenticate_frame, text="Enter otp", font="helvetica, 10").place(x=60, y=240)
            otp_entry = tk.Entry(self.authenticate_frame, width=40, font=("bold", 12))
            otp_entry.place(x=270, y=240)
            self.Gotp = tk.Button(self.authenticate_frame, text='Generate Otp', width=20, bg="SkyBlue1", fg='white',
                                     font=("bold", 12),
                                     command=lambda: [self.generateotp(Email_entry.get())])
            self.Gotp.place(x=240, y= 300)
            self.authrorise = tk.Button(self.authenticate_frame, text='Authenticate', width=20, bg="SkyBlue1", fg='white',
                                     font=("bold", 12),
                                     command=lambda: [self.loguser(Email_entry.get()), self.authorisefn(Email_entry.get(), otp_entry.get())])
            self.authrorise.place(x=240, y= 370)
            self.back_button = tk.Button(self.authenticate_frame, text='Go Back', width=20, bg="SkyBlue1", fg='white',
                                         font=("bold", 12),
                                         command=lambda: [self.show_frame(self.login_frame)])
            self.back_button.place(x=240, y=440)
        ...
        def passwordResetFrame():
            ...
            self.PwReset = tk.Button(self.passwordReset_frame, text='Submit', width=20, bg="SkyBlue1", fg='white',
                                     font=("bold", 12),
                                     command=lambda: [self.PwReset_data_validation(current_user, DOB_entry.get(),
                                                                                   New_Password.get(),
                                                                                   Confirm_password.get())])
            self.PwReset.place(x=240, y=450)
            self.back_button = tk.Button(self.passwordReset_frame, text='Go Back', width=20, bg="SkyBlue1", fg='white',
                                         font=("bold", 12),
                                         command=lambda: [self.show_frame(self.login_frame)])
            self.back_button.place(x=240, y=500)

        def AppMainPage(update_enrolled_courses=False):
            if collection.find_one({"Email": current_user}) is not None:
                if not update_enrolled_courses:
                    ...
                    tk.Button(self.main_frame, text=Stream[0], width=30, bg="pale turquoise", font="helvetica, 12",
                              command=lambda: [self.show_frame(self.gen_course_select_frame),
                                               self.logStream(Stream[0])]).place(
                        x=270, y=120)
                    ...
                    tk.Button(self.main_frame, text=Stream[2], width=30, bg="dark turquoise",
                              font="helvetica, 12",
                              command=lambda: [self.show_frame(self.nontec_course_select_frame),
                                               self.logStream(Stream[2])]).place(x=270, y=200)
                    tk.Label(self.main_frame, text="My Profile", font=("TrebuchetMS", 22, "bold"),
                             fg="DodgerBlue4").place(
                        x=1200, y=120)
                    msg1 = "Hello " + collection.find_one({"Email": current_user})["Firstname"] + ", welcome back"
                    tk.Label(self.main_frame, text=msg1, font=("TrebuchetMS", 14, "bold"),
                             fg="black").place(x=1150, y=170)
                    msg2 = "You last signed in on " + str(
                        collection.find_one({"Email": current_user})["Lastloggedintime"]) + " EST."
                    tk.Label(self.main_frame, text=msg2, font=("TrebuchetMS", 14),
                             fg="black").place(x=1050, y=205)
                    if len(collection.find_one({"Email": current_user})["CoursesEnrolled"]) < 10:
                        msg4 = "Total courses enrolled: 0" + str(
                            len(collection.find_one({"Email": current_user})["CoursesEnrolled"]))
                    else:
                        msg4 = "Total courses enrolled: " + str(
                            len(collection.find_one({"Email": current_user})["CoursesEnrolled"]))
                    tk.Label(self.main_frame, text=msg4, font=("TrebuchetMS", 14),
                             fg="black").place(x=1175, y=235)
                    obj = IcdeDataBase()
                    tk.Button(self.main_frame, text="My usage report", width=20, bg="green", fg="white",
                              font=("TrebuchetMS", 12, "bold"),
                              command=lambda: [IcdeDataBase.collect_usage_stats(obj, current_user)]).place(x=1170, y=275)
                    tk.Button(self.main_frame, text="Logout", width=20, bg="red", fg="white",
                              font=("TrebuchetMS", 12, "bold"),
                              command=lambda: [self.logSessionTime(time.time()), loginpage(),
                                               self.show_frame(self.login_frame),
                                               IcdeDataBase.update_usagetime(self, current_user, session_start_time,
                                                                             session_end_time)]).place(x=1170, y=315)
            if update_enrolled_courses:
                if len(collection.find_one({"Email": current_user})["CoursesEnrolled"]) < 10:
                    msg3 = "Total courses enrolled: 0" + str(
                        len(collection.find_one({"Email": current_user})["CoursesEnrolled"]))
                else:
                    msg3 = "Total courses enrolled: " + str(
                        len(collection.find_one({"Email": current_user})["CoursesEnrolled"]))
                tk.Label(self.main_frame, text=msg3, font=("TrebuchetMS", 14),
                         fg="black").place(x=1175, y=235)
        ...
        def EnrollmentPage():
            ...
            tk.Button(self.enroll_drop_frame, text="Enroll", width=30, bg="yellow green", font="helvetica, 12",
                      command=lambda: [self.enroll(current_user, selected_course),
                                       IcdeDataBase.add_course_user(self, current_user, selected_stream,
                                                                    selected_course)]).place(x=270, y=120)
            tk.Button(self.enroll_drop_frame, text="Drop", width=30, bg="indian red", font="helvetica, 12",
                      command=lambda: [self.drop(current_user, selected_course),
                                       IcdeDataBase.remove_course_user(self, current_user, selected_stream,
                                                                       selected_course)]).place(x=270, y=160)
            tk.Button(self.enroll_drop_frame, text="<-- Back", width=10, bg="light blue", font="helvetica, 12",
                      command=lambda: [AppMainPage(True), self.show_frame(self.main_frame)]).place(x=330, y=200)
        ...
        def Teccoursepage():
            ...
            tk.Button(self.tec_course_page, text=Stream[0], width=30, bg="SteelBlue1",
                      ...
                      command=lambda: [self.show_frame(self.tec_course_page),
                                       self.show_frame(self.elec_course_select_frame),
                                       self.logStream(Stream[0])]).place(x=270, y=120)
            tk.Button(self.tec_course_page, text=Stream[1], width=30, bg="SteelBlue2",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                             self.show_frame(self.electro_course_select_frame),
                                                             self.logStream(Stream[1])]).place(
                x=270, y=160)
            tk.Button(self.tec_course_page, text=Stream[2], width=30, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                             self.show_frame(self.comp_course_select_frame),
                                                             self.logStream(Stream[2])]).place(
                x=270, y=200)
            tk.Button(self.tec_course_page, text=Stream[3], width=30, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                             self.show_frame(self.civil_course_select_frame),
                                                             self.logStream(Stream[3])]).place(
                x=270, y=240)
            tk.Button(self.tec_course_page, text=Stream[4], width=30, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                             self.show_frame(self.mech_course_select_frame),
                                                             self.logStream(Stream[4])]).place(
                x=270, y=280)
            tk.Button(self.tec_course_page, text="<-- Back", width=10, bg="light blue", font="helvetica, 12",
                      command=lambda: [AppMainPage(True), self.show_frame(self.main_frame)]).place(x=350, y=380)
        ...
        def Courseselection():
            ...
            General_courses = ["Communication Skills",
            ...
            tk.Button(self.gen_course_select_frame, text=General_courses[3], width=80, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame),
                                                             self.logCourse(General_courses[3])]).place(x=270, y=240)

            tk.Button(self.gen_course_select_frame, text=General_courses[4], width=80, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame),
            ...
            tk.Button(self.gen_course_select_frame, text="<-- Back", width=10, bg="light blue", font="helvetica, 12",
                      command=lambda: [AppMainPage(True), self.show_frame(self.main_frame)]).place(x=550, y=400)
            tk.Button(self.gen_course_select_frame, text="Course analysis", width=80,
                      bg="SteelBlue1", font="helvetica, 12",
                      command=lambda: [IcdeDataBase.collect_courses_data(self, current_user, selected_stream)]).place(
                x=270, y=340)

            # ***********************************************************************************************************************************
            # Displaying Technical courses
            # Electrical Engineering
            Electrical_courses = ["EE1:Power Engineering",
                                  "EE2:Power system Analysis",
                                  "EE3:Control Systems",
                                  "EE4:Electrical Power Equipment",
                                  "EE5:Power Distribution"]
            ...
            tk.Button(self.elec_course_select_frame, text="<-- Back", width=10, bg="light blue", font="helvetica, 12",
                      command=lambda: [AppMainPage(True), self.show_frame(self.tec_course_page)]).place(x=550, y=400)
            ...
            Electronics_courses = ["EC1:Digital Logic Electronics",
                                   "EC2:Arduino Programming",
                                   "EC3:Industrial Robotics",
                                   "EC4:Computer architecture",
            ...
            Computer_courses = ["CE1:Learning Python",
                                "CE2:C Programming",
                                "CE3:Machine Learning",
                                "CE4:Data Structures and Algorithms",
                                "CE5:AWS Certification"]
        ...
        for frame in (self.home_frame, self.login_frame, self.signup_frame, self.passwordReset_frame, self.main_frame,
                      self.gen_course_select_frame, self.authenticate_frame,
