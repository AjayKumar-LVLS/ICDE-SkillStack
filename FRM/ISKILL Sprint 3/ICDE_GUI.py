...
class Functionalities(tk.Tk, Data_Verification):
    ...
    def New_Password_Validation(self,Newpwd,ConfirmPwd):
        flag_newpwd = self.confpwd(Newpwd)
        flag_confirmpwd = self.confpwd(ConfirmPwd)

        if flag_newpwd == flag_confirmpwd:
            tk.messagebox.showinfo("Message prompt", "Password reset is successful")
        else:
            tk.messagebox.showinfo("Error", "Password don't match")
...
class ICDEUser(Initialization, Functionalities):
    def __init__(self):
        ...
        def homepage():
            tk.Label(self.home_frame, text="ICDE - [Skill Stack]", width=100, font="bold, 30", bg="SkyBlue1", fg="DodgerBlue4",
                     height=2).pack()
        ...
        def loginpage():
            ...
            tk.Label(self.login_frame, text="Please Enter Login Credetails", width=100, font=("Papyrus", 30, "bold"), bg="SkyBlue1", fg="DodgerBlue4",
                     height=2).pack()
            ...
            tk.Label(self.login_frame, text="").pack()
            tk.Button(self.login_frame, text="Forgot Password", width=30, height=1, bg="SteelBlue2", font=("Cooper Black", 12),
        ...
        def passwordResetFrame():
            ...
            self.PwReset = tk.Button(self.passwordReset_frame, text='Submit', width=20, bg="black", fg='white',
                                     font=("bold", 12),
                                     command=lambda: [self.PwReset_data_validation(Email_entry.get(), DOB_entry.get()), self.confpwd(New_Password.get(), Confirm_password.get())])
            self.PwReset.place(x=240, y=450)

        def AppMainPage():
            self.refresh(self.main_frame)
            tk.Label(self.main_frame, text="Application Main Page", width=100, font=("TrebuchetMS", 30,"bold"), bg="SkyBlue1", fg="DodgerBlue4",
                     height=2).pack()
            ...
            tk.Button(self.main_frame, text="Non-technical Courses", width=30, bg="dark turquoise", font="helvetica, 12",
                      command=lambda: [self.show_frame(self.nontec_course_select_frame)]).place(x=270, y=200)

        #Creating Enrollment and Drop Page
        def EnrollmentPage():
            tk.Label(self.enroll_drop_frame, text="Enrollment / Drop Page", width=100, font="bold, 30", bg="SkyBlue1", fg="DodgerBlue4",
                     height=2).pack()
        ...
        def Teccoursepage():
            ...
            tk.Button(self.tec_course_page, text="Electrical Engineering", width=30, bg="SteelBlue1", font="Trebuchet, 12",
                      command=lambda: [self.show_frame(self.tec_course_page), self.show_frame(self.elec_course_select_frame)]).place(x=270, y=120)
            tk.Button(self.tec_course_page, text="Electronics Engineering", width=30, bg="SteelBlue2",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                            self.show_frame(self.electro_course_select_frame)]).place(x=270, y=160)
            tk.Button(self.tec_course_page, text="Computer Engineering", width=30, bg="SteelBlue3",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                            self.show_frame(self.comp_course_select_frame)]).place(x=270, y=200)
            tk.Button(self.tec_course_page, text="Civil Engineering", width=30, bg="SteelBlue4",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                            self.show_frame(self.civil_course_select_frame)]).place(x=270, y=240)
            tk.Button(self.tec_course_page, text="Mechanical Engineering", width=30, bg="DodgerBlue4",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.tec_course_page),
                                                            self.show_frame(self.mech_course_select_frame)]).place(x=270, y=280)
        ...
        def Courseselection():
            ...
            tk.Label(self.gen_course_select_frame, text="General Workshops", width=100, font="bold, 30", bg="SkyBlue1",
                     fg="DodgerBlue4",
                     height=2).pack()
            tk.Button(self.gen_course_select_frame, text="Spark! Articulate and Communicate your skills", width=80, bg="SteelBlue1",
                     font="helvetica, 12",command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=120)
            tk.Button(self.gen_course_select_frame, text="Essential Soft Skills for Career Success", width=80, bg="SteelBlue2",
                     font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=160)
            tk.Button(self.gen_course_select_frame, text="Guide to Entreprenuership", width=80, bg="SteelBlue3",
                     font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=200)
            tk.Button(self.gen_course_select_frame, text="Practicing Agility- Lean Start-Up and SCRUM Basics", width=80, bg="SteelBlue4",
                     font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=240)
            tk.Button(self.gen_course_select_frame, text="Library Skills & Resources- Maximize Your Graduate Research Potential", width=80,
                     bg="DodgerBlue4", font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=280)
        ...
        #***********************************************************************************************************************************
            ...
            tk.Button(self.elec_course_select_frame, text="EE1:Electrical Power Engineering Principles", width=80,
                      bg="lightgrey",font="helvetica, 12",command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=120)
            tk.Button(self.elec_course_select_frame, text="EE2:Power Engineering-Power system Analysis", width=80,
                      bg="lightgrey",font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=160)
            tk.Button(self.elec_course_select_frame, text="EE3:Electrical Control & Protection Systems", width=80, bg="lightgrey",
                      font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=200)
            tk.Button(self.elec_course_select_frame, text="EE4:Electrical Power Equipment",
                      width=80, bg="lightgrey", font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=240)
            tk.Button(self.elec_course_select_frame, text="EE5:Distribution Power Engineering Fundamentals", width=80,
                      bg="lightgrey", font="helvetica, 12", command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=280)
            ...
            tk.Button(self.electro_course_select_frame, text="EC1:Digital Electronics & Logic Design", width=80,
                      ...
                      command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=120)
            tk.Button(self.electro_course_select_frame, text="EC2:Arduino Programming and Hardware Fundamentals with Hackster", width=80,
                      bg="lightgrey", font="helvetica, 12",
                      command=lambda: [self.show_frame(self.enroll_drop_frame)]).place(x=270, y=160)
        ...
        #***********************************************************************************************************************************
            ...
            tk.Button(self.nontec_course_select_frame, text="French language", width=80,
                      bg="lightgrey", font="helvetica, 12",command=lambda: [self.show_frame(self.enroll_drop_frame)]).pack()
        ...
        EnrollmentPage()
        Teccoursepage()
        for frame in (self.home_frame, self.login_frame, self.signup_frame, self.passwordReset_frame, self.main_frame, self.gen_course_select_frame,
                      self.tec_course_select_frame, self.nontec_course_select_frame, self.enroll_drop_frame, self.tec_course_page, self.elec_course_select_frame,
                      self.electro_course_select_frame,self.comp_course_select_frame,self.civil_course_select_frame,self.mech_course_select_frame):
